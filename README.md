...
- **Language/runtime:** TypeScript, Node LTS, `tsx` for execution
- **LLM:** Anthropic Claude (`claude-sonnet-4-6`) via `@anthropic-ai/sdk`
- **Key dependencies:** `@anthropic-ai/sdk`, `ulid`, `ajv`, `ajv-formats`
- **Built with:** Claude Code (Anthropic's AI coding assistant)

## 3. Architecture

Each inbox item is processed by a **Claude-powered agentic loop** in `src/agent.ts`:

1. The item is sent to Claude with a system prompt encoding all policies and urgency calibration.
2. Claude decides which tools to call, returning tool-use blocks.
3. Each tool call is dispatched to the real `src/tools.ts` functions (which write the audit trace).
...
- Precision/recall on safeguarding detection (P0 true positives, false negatives).
- Tool call appropriateness scoring: penalize irrelevant tool calls and reward relevant ones.
- `draft_reply` quality: human review rubric (empathetic, no clinical advice, no implied sending).
- Regression suite: run the validator on every commit with a known-good fixture.
